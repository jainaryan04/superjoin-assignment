...
def render_fact_details(fact: dict) -> None:
    ...
    info[1].markdown(f"**Canonical Entity**  \n{fact.get('canonical_entity') or '—'}")
    info[2].markdown(f"**Raw Attribute**  \n{fact.get('raw_attribute') or '—'}")
    info[3].markdown(f"**Canonical Attribute**  \n{fact.get('canonical_attribute') or fact.get('attribute') or '—'}")
    info[4].markdown(f"**Value**  \n{fact.get('value') or '—'}")
    info[5].markdown(f"**Canonical Value**  \n{fact.get('canonical_value') or '—'}")
    info[6].markdown(f"**Period**  \n{fact.get('period') or '—'}")
